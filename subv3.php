    ...
    <script>
        jQuery(document).ready(function($) {
            $('#billing_method').change(function() {
                ...
                if (selectedValue === '1') {
                    $('#subscription_options_container').show();
                } else {
                    $('#subscription_options_container').hide();
                    $('#selected_subscription').val(''); // Reset selection
                }
            }).change(); // Trigger change to set initial state

            $('#subscription_options').change(function() {
                var selectedValue = $(this).val();
                $('#selected_subscription').val(selectedValue);
            });
        });
    </script>
...
function wk_add_to_cart_validation( $passed, $product_id, $quantity, $variation_id = null ) {
    ...
    $subscription_billing = isset($_POST['selected_subscription']) && !empty($_POST['selected_subscription']);

    if ($is_subscription_product === 'yes' && !$subscription_billing) {
        $passed = false;
        wc_add_notice('Please select a subscription option before adding to cart.', 'error');
    }
